...
# ================================================================
# ⭐ UNIVERSAL TARGET SELECTION (UPDATED — FIXED)
# ================================================================
def identify_target(columns, df):
    """
    Universal safe target selector for ANY life-science dataset.
    Prevents selecting noisy targets like Gender, ID, Name, Age.
    """

    BAD_TARGETS = ["gender", "sex", "id", "patientid", "name", "age"]

    # 1) Prefer columns that explicitly describe labels
    preferred = ["target", "diagnosis", "outcome", "label", "stage", "status", "class"]
    lower = [c.lower() for c in columns]

    for key in preferred:
        if key in lower:
            return columns[lower.index(key)]

    # 2) Select categorical columns with 2–5 unique values (best classification targets)
    for c in columns:
        if c.lower() in BAD_TARGETS:
            continue
        if 2 <= df[c].nunique() <= 5:
            return c

    # 3) Fallback: numeric but low-unique columns
...
TEMPLATES_REJ = {
    "oncology": ["{} overlaps with stronger tumor descriptors and adds limited new biological insight."],
    "cardiology": ["{} is lightly tied to cardiovascular physiology compared to retained markers."],
    "hepatology": ["{} does not directly reflect liver pathophysiology and was deprioritized."],
    "general": ["{} contributes minimal direct biological interpretation relative to selected markers."]
}
...
def analyze_file(path, keep_fraction=0.6):
    ...
    # Fix: prevent auto-selecting very noisy targets
